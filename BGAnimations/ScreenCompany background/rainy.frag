...
{
    float resolution = 10. * exp2(-3.*iMouse.x/imageSize.x);
	vec2 uv = fragCoord.xy / imageSize.xy * resolution;
    vec2 p0 = floor(img2tex(uv));
    ...
    vec3 color = texture2D(sampler0, img2tex(uv)/resolution - intensity*n.xy).rgb + 5.*pow(clamp(dot(n, normalize(vec3(1., 0.7, 0.5))), 0., 1.), 6.);
	fragColor = vec4(color, amt);
}
